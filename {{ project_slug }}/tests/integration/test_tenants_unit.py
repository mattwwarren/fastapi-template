...
from {{ project_slug }}.core.tenants import (
    TenantContext,
    TenantIsolationMiddleware,
...
class TestTenantIsolationMiddleware:
    ...
    async def test_skips_when_enforcement_disabled(self) -> None:
        ...
        with patch("{{ project_slug }}.core.tenants.settings.enforce_tenant_isolation", False):
            response = await middleware.dispatch(mock_request, mock_call_next)
    ...
    async def test_returns_401_when_no_user(self) -> None:
        ...
        with patch("{{ project_slug }}.core.tenants.settings.enforce_tenant_isolation", True):
            response = await middleware.dispatch(mock_request, mock_call_next)
