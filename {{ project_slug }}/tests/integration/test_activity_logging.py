...
from {{ project_slug }}.core.activity_logging import (
    log_activity,
...
class TestLogActivityDisabled:
    ...
    async def test_log_activity_disabled_returns_early(
        ...
        with patch("{{ project_slug }}.core.activity_logging.settings", mock_settings):
...
class TestLogActivityFireAndForget:
    ...
    async def test_log_activity_without_session_uses_internal_session(
        ...
        with patch(
            "{{ project_slug }}.core.activity_logging.async_session_maker",
            mock_session_maker,
        ):
