...
def validate_pagination_response(
    response_data: dict[str, Any], min_items: int = 0, max_items: int = 1000
) -> None:
...
def assert_error_response(
    response_data: dict[str, Any], expected_detail: str | None = None
) -> None:
    ...
    assert isinstance(
        response_data["detail"], (str, list)
    ), "'detail' must be string or list"

    if expected_detail is not None:
        if isinstance(response_data["detail"], str):
            assert (
                expected_detail in response_data["detail"]
            ), f"Expected detail '{expected_detail}' in '{response_data['detail']}'"
        else:
            # Handle validation error details (list of errors)
            detail_strs = [
                d if isinstance(d, str) else d.get("msg", "")
                for d in response_data["detail"]
            ]
            assert any(
                expected_detail in d for d in detail_strs
            ), f"Expected '{expected_detail}' in error details"
