...
from {{ project_slug }}.core.logging import (
    LoggingMiddleware,
    _org_id_var,
    _request_id_var,
    _user_id_var,
...
class TestLoggingMiddleware:
    ...
    async def test_generates_request_id_when_not_provided(self) -> None:
        ...
        with patch("{{ project_slug }}.core.logging.settings") as mock_settings:
    ...
    async def test_logs_request_started_when_enabled(self) -> None:
        ...
        with (
            patch("{{ project_slug }}.core.logging.settings") as mock_settings,
            patch("{{ project_slug }}.core.logging.LOGGER") as mock_logger,
        ):
            mock_settings.request_id_header = "x-request-id"
