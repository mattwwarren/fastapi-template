...
from {{ project_slug }}.db.session import get_session, init_db
...
class TestGetSession:
    ...
    async def test_session_rollback_on_exception(self) -> None:
        ...
        with patch("{{ project_slug }}.db.session.async_session_maker", mock_session_maker):
            gen = get_session()
...
class TestInitDb:
    ...
    async def test_init_db_uses_global_engine_when_none_provided(self) -> None:
        ...
        with patch("{{ project_slug }}.db.session.engine", mock_engine):
            await init_db()
