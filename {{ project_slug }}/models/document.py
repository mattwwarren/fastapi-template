...
For provider setup instructions, see:
    - {{ project_slug }}/core/storage.py (abstraction layer)
    - {{ project_slug }}/core/storage_providers.py (implementation details)
"""
...
class Document(TimestampedTable, DocumentBase, table=True):
    ...
    storage_path: str = Field(
        description="Storage path/key where file is stored in object storage"
    )
    storage_url: str = Field(
        description="URL for accessing the file (may be presigned/temporary)"
    )
