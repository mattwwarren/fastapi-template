...
cd /tmp && rm -rf test_{{ project_slug }}
mkdir -p test_{{ project_slug }}

# Generate from template using copier
copier copy /path/to/fastapi-template test_{{ project_slug }} \
  --data project_slug=test_project \
  --data "project_name=Test Project" \
  --defaults

# Verify project structure
cd test_{{ project_slug }}
ls -la
```
...
TEST_DIR="/tmp/test_{{ project_slug }}_$(date +%s)"
