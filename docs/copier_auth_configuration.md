...
The {{ project_slug }} supports multiple authentication providers. This document explains how to configure authentication for your deployment.
...
The settings are defined in `{{ project_slug }}/core/config.py`:
...
Authentication middleware is configured in `{{ project_slug }}/main.py`:
...
- [Auth Configuration](../{{ project_slug }}/core/auth.py) - Authentication implementation
