...
- Main branch contains working Python code (`{{ project_slug }}/`)
...
2. **Work directly on main branch**
   ```bash
   # Make your changes to {{ project_slug }}/
...
2. The workflow generates `{{ project_slug }}` variables from `{{ project_slug }}/`
...
```
fastapi-template/
├── {{ project_slug }}/         # Runnable Python package (main branch)
│   ├── api/
│   ├── models/
│   ├── services/
│   └── main.py
...
# Contribute to template: {{ project_slug }}/models/validators.py
